...
| `GET` | `/api_gateway/v1/users/validate` | access token | Validates the access token. If `X-Service-ID` is present, also checks `usr_info.usr_access`. Used by NGINX `auth_request`. | `backend/users/urls.py`, `backend/users/views.py`, `backend/utils/jwt.py`, `extra/example.conf` |
